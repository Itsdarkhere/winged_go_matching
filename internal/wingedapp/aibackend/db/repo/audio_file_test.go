...
type testCaseAudioFile struct {
	name           string
	filter         *registration.AudioFileQueryFilter
	mutations      func(th *testsuite.Helper, db boil.ContextExecutor, tc *testCaseAudioFile)
	assertions     func(th *testsuite.Helper, db boil.ContextExecutor, tc *testCaseAudioFile, audioFile aipgmodel.AudioFileSlice, err error)
	expectedID     string
	expectedUserID string
	expectedIDs    []string
}

func getTestCasesAudioFile() []testCaseAudioFile {
	return []testCaseAudioFile{
		{
